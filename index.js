...
// 宽容版：检测 persona 是否绑定到任何角色（覆盖 ST 各版本数据格式）
function isBound(a) {
    // 1. persona_descriptions[avatar].position === 'character'
    const desc = (power_user.persona_descriptions || {})[a];
    if (desc) {
        if (desc.position === 'character') return true;
        // 1.13+ 新版用 connections 数组保存绑定关系
        if (Array.isArray(desc.connections) && desc.connections.length > 0) return true;
        if (desc.lockedFor && Array.isArray(desc.lockedFor) && desc.lockedFor.length > 0) return true;
    }
    // 2. personas_lock 或 lockedPersonas（不同版本字段名不同）
    const lockObjs = [
        power_user.personas_lock,
        power_user.lockedPersonas,
        power_user.persona_lock,
    ];
    for (const lock of lockObjs) {
        if (!lock || typeof lock !== 'object') continue;
        // 格式 A：{ avatar: charAvatar }，key 是 persona
        if (lock[a] !== undefined && lock[a] !== null && lock[a] !== '') return true;
        // 格式 B：{ charAvatar: personaAvatar }，value 是 persona
        for (const k in lock) {
            if (lock[k] === a) return true;
        }
    }
    // 3. 顶层字段（更老版本）
    if (Array.isArray(power_user.persona_locked_chats) && power_user.persona_locked_chats.includes(a)) return true;
...
function renderCard(avatar) {
    ...
    card.addEventListener('click', e => {
        ...
        document.querySelectorAll('.pg-persona-card.pg-current').forEach(el => el.classList.remove('pg-current'));
        document.querySelectorAll('.pg-quick-avatar.pg-current').forEach(el => el.classList.remove('pg-current'));
        card.classList.add('pg-current');
...
function initQuick() {
    const tryInject = () => {
        ...
        btn.tabIndex = 0;
        btn.innerHTML = '<img class="pg-quick-btn-img" alt=""><i class="fa-solid fa-user-circle pg-fallback-icon" style="display:none;"></i>';
        // 学 Quick Persona：直接 append 到 leftSendForm，靠 CSS order 控制位置
        leftForm.appendChild(btn);
        btn.addEventListener('click', toggleQuick);
...
function updateQuickBtnAvatar() {
    ...
    if (!cur) {
        const selected = document.querySelector('#user_avatar_block .avatar-container.selected [data-avatar-id]')
                      || document.querySelector('#user_avatar_block [data-avatar-id].selected');
        if (selected) cur = selected.dataset.avatarId;
    }
    if (!cur) {
        const first = document.querySelector('#user_avatar_block [data-avatar-id]');
        if (first) cur = first.dataset.avatarId;
...
function renderQuick() {
    ...
    p.querySelectorAll('.pg-quick-avatar').forEach(el => el.addEventListener('click', () => {
        const a = el.dataset.avatar;
        document.querySelectorAll('.pg-persona-card.pg-current').forEach(x => x.classList.remove('pg-current'));
        document.querySelectorAll('.pg-quick-avatar.pg-current').forEach(x => x.classList.remove('pg-current'));
        el.classList.add('pg-current');
        switchPersona(a);
        setTimeout(updateQuickBtnAvatar, 50);
